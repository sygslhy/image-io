...
template <typename T>
Image<T> createImageFromPyarray(py::array_t<T> b,
                                PixelType pixelType,
    ...
    if (imageLayout == ImageLayout::YUV_420 || imageLayout == ImageLayout::NV12) {
        LayoutDescriptor layout = LayoutDescriptor::Builder(info.shape[1], info.shape[0] * 2 / 3)
...
void init_image(py::module &m) {
    ...
    py::class_<Image<int>, ImageView<int>>(m, "ImageInt", py::buffer_protocol())
            ...
            .def_buffer([](Image<int> &m) -> py::buffer_info { return defineBufferInfos(m); });
    ...
    py::class_<Image<float>, ImageView<float>>(m, "ImageFloat", py::buffer_protocol())
            ...
            .def_buffer([](Image<float> &m) -> py::buffer_info { return defineBufferInfos(m); });
    ...
    py::class_<Image<uint8_t>, ImageView<uint8_t>>(m, "ImageUint8", py::buffer_protocol())
            ...
            .def_buffer([](Image<uint8_t> &m) -> py::buffer_info { return defineBufferInfos(m); });
    ...
    py::class_<Image<uint16_t>, ImageView<uint16_t>>(m, "ImageUint16", py::buffer_protocol())
            .def(py::init(
                    [](py::array_t<uint16_t> b, PixelType pixelType, ImageLayout imageLayout, int pixelPrecision) {
                        return createImageFromPyarray<uint16_t>(b, pixelType, imageLayout, pixelPrecision);
                    }))
            .def_buffer([](Image<uint16_t> &m) -> py::buffer_info { return defineBufferInfos(m); });
    ...
    py::class_<Image<double>, ImageView<double>>(m, "ImageDouble", py::buffer_protocol())
            ...
            .def_buffer([](Image<double> &m) -> py::buffer_info { return defineBufferInfos(m); });
}
