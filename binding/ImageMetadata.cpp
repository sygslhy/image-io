...
void init_model(py::module &m) {
    ...
    py::class_<ImageMetadata> imageMetadata(
            m, "ImageMetadata", py::is_final(), "Image Metadata, see class detail with help(ImageMetadata)");
    ...
    py::class_<ImageMetadata::FileInfo>(imageMetadata,
            ...
            .def("serialize",
                 [](const ImageMetadata::FileInfo &fileInfo) {
                     py::dict dict;
                     if (fileInfo.width) dict["width"] = *fileInfo.width;
                     if (fileInfo.height) dict["height"] = *fileInfo.height;
                     if (fileInfo.pixelPrecision) dict["pixelPrecision"] = *fileInfo.pixelPrecision;
                     if (fileInfo.fileFormat) dict["fileFormat"] = py::str(cxximg::toString(*fileInfo.fileFormat));
                     if (fileInfo.imageLayout) dict["imageLayout"] = py::str(cxximg::toString(*fileInfo.imageLayout));
                     if (fileInfo.pixelType) dict["pixelType"] = py::str(cxximg::toString(*fileInfo.pixelType));
                     if (fileInfo.pixelRepresentation)
                         dict["pixelRepresentation"] = py::str(cxximg::toString(*fileInfo.pixelRepresentation));
                     if (fileInfo.widthAlignment) dict["widthAlignment"] = *fileInfo.widthAlignment;
                     if (fileInfo.heightAlignment) dict["heightAlignment"] = *fileInfo.heightAlignment;
                     if (fileInfo.sizeAlignment) dict["sizeAlignment"] = *fileInfo.sizeAlignment;
                     return dict;
                 })
    ...
    py::class_<ImageMetadata::ShootingParams>(
            ...
            .def("serialize",
                 [](const ImageMetadata::ShootingParams &shootingParams) {
                     py::dict dict;
                     if (shootingParams.aperture) dict["aperture"] = py::float_(*shootingParams.aperture);
                     if (shootingParams.exposureTime) dict["exposureTime"] = py::float_(*shootingParams.exposureTime);
                     if (shootingParams.sensitivity) dict["sensitivity"] = py::float_(*shootingParams.sensitivity);
                     if (shootingParams.totalGain) dict["totalGain"] = py::float_(*shootingParams.totalGain);
                     if (shootingParams.sensorGain) dict["sensorGain"] = py::float_(*shootingParams.sensorGain);
                     if (shootingParams.ispGain) dict["ispGain"] = py::float_(*shootingParams.ispGain);
                     if (shootingParams.zoom) dict["zoom"] = py::cast(shootingParams.zoom).attr("serialize")();
                     return dict;
                 })
    ...
    py::class_<ImageMetadata::CalibrationData>(
            ...
            .def("serialize",
                 [](const ImageMetadata::CalibrationData calibData) {
                     py::dict dict;
                     if (calibData.blackLevel) dict["blackLevel"] = *calibData.blackLevel;
                     if (calibData.whiteLevel) dict["whiteLevel"] = *calibData.whiteLevel;
                     if (calibData.vignetting) dict["vignetting"] = py::cast(calibData.vignetting).attr("serialize")();
